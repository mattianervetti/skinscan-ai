...
Modello linguistico: Google Gemini (gemini-2.5-flash), tramite langchain-google-genai.
Regola SSL/chiave: tutte le chiamate al modello linguistico passano da un unico modulo dedicato. È quel modulo, e solo quello, a occuparsi della configurazione SSL (truststore.inject_into_ssl(), racchiuso in un try/except così che su Linux/Streamlit Cloud un eventuale fallimento non blocchi l'avvio) e della lettura della chiave da .env. Nessun altro file del progetto deve importare truststore o leggere direttamente GOOGLE_API_KEY.

## 5. Gli agenti (LangGraph)
1. ACCOGLIENZA — valuta il questionario (fototipo, numero di nei, familiarità, melanoma pregresso, immunosoppressione, neo che cambia) e assegna priorità alta/media/bassa. Se rischio basso e nessun sintomo: solo consigli di prevenzione e promemoria per l'autoesame, nessun esame.
