...
4. INSTRADAMENTO — mette il caso nella coda del dermatologo per priorità, prenota la televisita su un'agenda simulata, invia notifiche simulate visibili nell'app, sollecita chi non conferma e scala a un operatore umano dopo 2 solleciti. Se serve biopsia, prenota nel centro convenzionato simulato. → COMPLETATO.
   ...
   Nessuna nuova tabella: le tabelle notifiche e appuntamenti esistevano già nello schema dalla Fase 1 (VERSIONE_SCHEMA invariata a 1). Pagina Dermatologo collegata alla coda reale; pagina Paziente estesa con la sezione "Televisita" (proposta, conferma, simulazione di sollecito per la demo dal vivo).
...
2. Agenti uno alla volta (1→5), ciascuno testato su un caso demo prima del successivo. → IN CORSO (4/5)
   ...
   Agente 4 (INSTRADAMENTO) completato — vedi dettagli in sezione 5. Nessuna tabella nuova (notifiche e appuntamenti erano già nello schema dalla Fase 1). Pagina Dermatologo: da segnaposto a coda reale ordinata per priorità. Pagina Paziente: nuova sezione "Televisita" (proposta, conferma dal paziente, pulsante di simulazione sollecito per la demo dal vivo). Verificati i 4 casi demo: Marta in cima alla coda, Giulia in coda nonostante l'analisi rassicurante (neo cambiato), Luca non entra in coda, Paolo riceve 2 solleciti poi è scalato a un operatore umano.
   NOTA APERTA (non risolta in questo passaggio, non riguarda l'agente 4): nucleo/modello_linguistico.py ha una modifica non ancora committata (import di ChatGoogleGenerativeAI spostato dentro chiedi_al_modello, per non caricarlo quando DISATTIVA_MODELLO=true ed evitare un possibile blocco di Windows Defender Application Control) che rompe test/test_modello_linguistico.py (il test cerca di sostituire nucleo.modello_linguistico.ChatGoogleGenerativeAI a livello di modulo, ma con l'import spostato dentro la funzione quell'attributo non esiste più a livello di modulo). Da sistemare prima del prossimo commit che tocchi quel file.
